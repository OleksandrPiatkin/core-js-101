 ...
 *   'cat'       => 'c'
 */
const getFirstChar = (str) => str.split('').shift();
...
const getRectangleString = (width, height) => {
  const line = (type) => type.repeat(width - 2);
  const top = `┌${line('─')}┐\n`;
  const bot = `└${line('─')}┘\n`;
  const middle = height > 2 ? `│${line(' ')}│\n`.repeat(height - 2) : '';
...
const encodeToRot13 = (str) => {
  ...
  return lettersArr.reduce((acc, val) => (val.match(/[a-z]/i) ? acc + output[input.indexOf(val)] : acc + val), '');
};
 ...
 */
const isString = (value) => (value instanceof String ? true : typeof value === 'string');
