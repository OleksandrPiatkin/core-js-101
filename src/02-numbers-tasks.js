 ...
 *   5*x = 0         => 0
 */
const getLinearEquationRoot = (a, b) => (a === 0 ? -b : -b / a);
...
const getAngleBetweenVectors = (x1, y1, x2, y2) => {
  const xyCoor = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x1 ** 2 + y1 ** 2);
  return Math.acos((x1 * x2 + y1 * y2) / xyCoor);
};
 ...
 * '-525.5'     => -525.5
 */
const parseNumberFromString = (value) => value.toString();
 ...
 */
const toNumber = (value, def) => (+value ? +value : def);
